...
function NowPlayingSection({ item, progressMs, context, contextPlaylist }) {
  ...
  const playlistUrl =
    playlist?.url || (context?.type === 'playlist' ? context?.external_urls?.spotify : null);
      ...
      {playlistUrl && (
        ...
        >
          {playlistImg ? (
            ...
            >
              <svg viewBox="0 0 24 24" fill={SP.faint} width="20" height="20">
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            ...
            {playlistTotal != null && (
              <div style={{ fontSize: '11px', color: SP.faint, marginTop: '2px' }}>
                {playlistTotal} tracks
              </div>
            )}
          </div>
          <svg
...
function RecentSection({ items }) {
  if (!items?.length)
    return <p style={{ color: SP.faint, fontSize: '13px', margin: 0 }}>nothing here yet</p>;
  return (
      ...
      {items.slice(0, 8).map((item, i) => {
        ...
        return (
          <div
            ...
            style={{ display: 'flex', gap: '10px', alignItems: 'center', padding: '6px 8px' }}
          >
            ...
            <span
              style={{ fontSize: '10px', color: SP.faint, flexShrink: 0, whiteSpace: 'nowrap' }}
            >
...
function PlaylistsSection({ items }) {
  if (!items?.length)
    return <p style={{ color: SP.faint, fontSize: '13px', margin: 0 }}>no playlists</p>;
  return (
    <div
      style={{
        display: 'grid',
    ...
    >
      {items.map((pl) => (
        <a
          ...
          target="_blank"
          rel="noopener noreferrer"
          className="sp-pl-cell"
          style={{ display: 'flex', flexDirection: 'column', gap: '7px', textDecoration: 'none' }}
        >
          {pl.images?.[0]?.url ? (
            <img
              src={pl.images[0].url}
            ...
            >
              <svg viewBox="0 0 24 24" fill={SP.faint} width="28" height="28">
                <path d="M15 6H3v2h12V6zm0 4H3v2h12v-2zM3 16h8v-2H3v2zM17 6v8.18c-.31-.11-.65-.18-1-.18-1.66 0-3 1.34-3 3s1.34 3 3 3 3-1.34 3-3V8h3V6h-5z" />
              </svg>
            </div>
          )}
          <div>
            ...
            {pl.tracks?.total != null && (
              <div style={{ fontSize: '10px', color: SP.faint, marginTop: '2px' }}>
...
function TopTracksSection({ items }) {
  if (!items?.length)
    return <p style={{ color: SP.faint, fontSize: '13px', margin: 0 }}>no data yet</p>;
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '2px' }}>
      ...
      {items.slice(0, 10).map((a) => (
        ...
        >
          {a.images?.[1]?.url ? (
            ...
            >
              <svg viewBox="0 0 24 24" fill={SP.faint} width="26" height="26">
...
export function SpotifySimpleCard({ userId }) {
  ...
  return (
    ...
    >
      <SpIcon size={20} />
      <div>
        <div style={{ fontWeight: '700', fontSize: '14px', color: SP.green, lineHeight: 1.2 }}>
          spotify
        </div>
...
export function SpotifyCard({ userId, apiEndpoint }) {
  ...
  const { loading, error } = usePolledJSON(apiEndpoint, 30000, (d, initial) => {
    ...
    setData((prev) => {
      ...
      if (
        ...
        (nowPlaying && prev?.status?.item?.id !== d.status?.item?.id)
      ) {
  ...
  useEffect(() => {
    ...
    const t = setInterval(
      () => setProgressMs((p) => (duration > 0 ? Math.min(p + 1000, duration) : p + 1000)),
      1000,
    );
    return () => clearInterval(t);
  }, [isPlaying, currentItem?.id, currentItem?.duration_ms]);
  ...
  return (
    ...
    >
      ...
      <div
        style={{
          background: SP.bgHead,
          padding: '10px 14px',
          display: 'flex',
          alignItems: 'center',
          gap: '8px',
          borderBottom: collapsed ? 'none' : `1px solid ${SP.border}`,
        }}
      >
      ...
      <div className={`sc-body ${collapsed ? 'closed' : 'open'}`}>
        ...
        {profile && (
          <div
            className="sp-profile-section"
            style={{
              padding: '16px 20px',
              borderBottom: `1px solid ${SP.div}`,
              display: 'flex',
              alignItems: 'center',
              gap: '14px',
            }}
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              ...
              {profile.followers?.total != null && (
                <div style={{ fontSize: '12px', color: SP.muted, marginTop: '4px' }}>
                  <span style={{ color: SP.white, fontWeight: '600' }}>
        ...
        {isPlaying && currentItem && (
          <div
            className="sp-nowplaying-pad"
            style={{ padding: '20px', borderBottom: `1px solid ${SP.div}` }}
          >
            <NowPlayingSection
              item={currentItem}
        ...
        {data && !isPlaying && (
          ...
          >
            ...
            <span style={{ fontSize: '12px', color: SP.faint }}>
              ...
              {TABS.map((t) => (
                <button
                  ...
                  style={{
                    ...
                    color: tab === t.id ? SP.white : SP.faint,
                    background: 'none',
                    border: 'none',
                    cursor: 'pointer',
                    borderBottom: `2px solid ${tab === t.id ? SP.green : 'transparent'}`,
                    textTransform: 'uppercase',
